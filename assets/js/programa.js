(function () {
  ...
  var currentAgendaData = null;
  var fuentesPorNombre = {};
  ...
  function aplicarMesDesdeInput(mesValor) {
    if (!mesValor || mesValor.length < 7) return;
    var parts = mesValor.split("-");
    var y = parseInt(parts[0], 10);
    var m = parseInt(parts[1], 10) - 1;
    var ini = y + "-" + parts[1] + "-01";
    var fin =
      y +
      "-" +
      parts[1] +
      "-" +
      String(ultimoDiaMes(y, m)).padStart(2, "0");
    if (inputDesde) inputDesde.value = ini;
    if (inputHasta) inputHasta.value = fin;
  }
  ...
  function cambiarMes(delta) {
    ...
    aplicarMesDesdeInput(inputMes.value);
    cargarPublicaciones().catch(function () {});
  }
  ...
  function cargarEstado() {
    ...
    fetch("/api/programa/estado")
      ...
      .then(function (d) {
        ...
        if (statCola && d.cola_pendientes != null) {
          statCola.textContent = d.cola_pendientes;
        }
  ...
  function vaEnCeldaCalendario(it) {
    ...
    if (dias > 21 && tiposLargos.indexOf(it.tipo) < 0) return false;
    return true;
  }
  ...
  function buildSourceMap(fuentes) {
    var map = {};
    if (!fuentes || typeof fuentes !== "object") return map;
    Object.keys(fuentes).forEach(function (k) {
      var arr = fuentes[k];
      if (!Array.isArray(arr)) return;
      arr.forEach(function (it) {
        if (!it || !it.nombre || !it.url) return;
        map[normalizarTexto(it.nombre)] = it.url;
        if (it.id) map[normalizarTexto(it.id)] = it.url;
      });
    });
    return map;
  }

  function resolveSourceUrl(ev) {
    if (!ev) return "";
    if (ev.fuente_url) return ev.fuente_url;
    var f = normalizarTexto(ev.fuente || "");
    if (!f) return "";
    if (f.indexOf("cordoba_turismo") >= 0 && fuentesPorNombre["cordoba_turismo"]) {
      return fuentesPorNombre["cordoba_turismo"];
    }
    if (f.indexOf("kempes") >= 0 && fuentesPorNombre["estadio_kempes"]) {
      return fuentesPorNombre["estadio_kempes"];
    }
    return fuentesPorNombre[f] || "";
  }
  ...
  function detalleEventoHtml(ev) {
    ...
    if (ev.semanas_calendario_escolar)
      h += "<p><strong>Semanas escolares:</strong> " + escHtml(ev.semanas_calendario_escolar) + "</p>";
    if (ev.mensaje_campana)
      h += "<p><strong>Ángulo comercial:</strong> " + escHtml(ev.mensaje_campana) + "</p>";
    if (ev.estado) h += "<p><em>Estado: " + escHtml(ev.estado) + "</em></p>";
    if (ev.fuente) {
      var srcUrl = resolveSourceUrl(ev);
      h += "<p style='font-size:0.8rem;opacity:0.85'>Fuente: ";
      if (srcUrl) {
        h += "<a href='" + escHtml(srcUrl) + "' target='_blank' rel='noopener'>" + escHtml(ev.fuente) + "</a>";
      } else {
        h += escHtml(ev.fuente);
      }
      h += "</p>";
  ...
  function llenarCeldaCalendario(cel, iso, evs, infoPuente) {
    ...
    if (infoPuente) {
      ...
      var vacVer = evs.find(function (e) {
        return e.tipo === "vacaciones_verano";
      });
      var vacInv = evs.find(function (e) {
        return e.tipo === "vacaciones_invierno";
      });
      if (vacVer) {
        var etV = document.createElement("span");
        etV.className = "cal-banda-etiqueta cal-banda-verano";
        etV.textContent = "VERANO ESCOLAR";
        cel.appendChild(etV);
      }
      if (vacInv) {
        var etI = document.createElement("span");
        etI.className = "cal-banda-etiqueta cal-banda-invierno";
        etI.textContent = (vacInv.nombre || "Invierno").replace("Vacaciones invierno — ", "");
        cel.appendChild(etI);
      }
      var tipos = {};
      ...
      else if (partes.length) resumen.textContent = partes.join(" · ");
      if (resumen.textContent) cel.appendChild(resumen);
    ...
    while (y < finY || (y === finY && m <= finM)) {
      ...
      for (var dia = 1; dia <= ultimoDia.getDate(); dia++) {
        ...
        if (
          ...
          cel.classList.add("es-vacaciones-verano");
        if (
          evs.some(function (e) {
            return e.tipo === "vacaciones_invierno";
          })
        )
          cel.classList.add("es-vacaciones-invierno");
  ...
  function renderLeyenda() {
    ...
    if (!el) return;
    el.innerHTML =
      '<span class="lg-puente">Azul · finde largo (cada día del puente)</span>' +
      '<span class="lg-verano">Naranja · vacaciones verano (20 dic – 28 feb)</span>' +
      '<span class="lg-invierno">Violeta · vacaciones invierno por provincia</span>' +
      '<span class="lg-temporada-alta">Rojo suave · temporada alta</span>' +
      '<span class="lg-temporada-media">Turquesa · temporada media</span>' +
      '<span class="lg-temporada-baja">Gris · temporada baja</span>' +
      '<span class="lg-feriado">Borde rojo · feriado nacional</span>' +
      '<span class="lg-evento">Dorado · evento confirmado</span>' +
      "<span>Tocá un día para ver detalle</span>";
  }

  function seasonByDate(iso) {
    if (!iso || iso.length < 10) return "baja";
    var month = parseInt(iso.slice(5, 7), 10);
    if (month === 12 || month === 1 || month === 2 || month === 7) return "alta";
    if (month === 3 || month === 4 || month === 10 || month === 11) return "media";
    return "baja";
  }

  function renderYearGrid(d) {
    var wrap = document.getElementById("cal-preview-year");
    if (!wrap) return;
    wrap.innerHTML = "";
    var porDia = indicePorDia(d.items || []);
    var from = parseIsoLocal(d.desde);
    var to = parseIsoLocal(d.hasta);
    var cur = new Date(from.getFullYear(), 0, 1);
    var endMonth = new Date(to.getFullYear(), to.getMonth(), 1);
    ...
    while (cur <= endMonth) {
      ...
      header.textContent = MESES[m + 1] + " " + y;
      card.appendChild(header);

      var head = document.createElement("div");
      head.className = "year-week-head";
      DIAS_SEM.forEach(function (lbl) {
        var s = document.createElement("span");
        s.textContent = lbl;
        head.appendChild(s);
      ...
      for (var day = 1; day <= last; day++) {
        ...
        var events = porDia[iso] || [];
        var el = document.createElement("button");
        el.type = "button";
        el.className = "year-day season-" + seasonByDate(iso);
        if (events.length) el.classList.add("with-event");
        if (events.some(function (e) { return e.tipo === "finde_largo"; })) el.classList.add("kind-bridge");
        if (events.some(function (e) { return e.tipo === "feriado_nacional"; })) el.classList.add("kind-holiday");
        el.textContent = String(day);
        var tt = [fmtDDMMYYYY(iso)];
        events.slice(0, 4).forEach(function (ev) {
          tt.push("- " + (ev.nombre || ""));
        });
        if (events.length > 4) tt.push("- +" + (events.length - 4) + " eventos");
        el.title = tt.join("\n");
        (function (isoKey, cell) {
          cell.addEventListener("click", function () {
            var evsClick = porDia[isoKey] || [];
            abrirDetalleDia(isoKey, evsClick);
          });
        })(iso, el);
        grid.appendChild(el);
  ...
  function renderFuentesDb(d) {
    var panel = document.getElementById("panel-fuentes-db");
    var lista = document.getElementById("lista-fuentes-db");
    if (!panel || !lista) return;
    var fuentes = d.fuentes || {};
    var bloques = [];
    Object.keys(fuentes).forEach(function (k) {
      if (!Array.isArray(fuentes[k]) || !fuentes[k].length) return;
      var titulo = k.replace(/_/g, " ");
      var html = "<section class='fuente-bloque'><h4>" + escHtml(titulo) + "</h4><ul>";
      fuentes[k].forEach(function (f) {
        html += "<li><strong>" + escHtml(f.nombre || f.id || "Fuente") + "</strong>";
        if (f.url) html += " · <a href='" + escHtml(f.url) + "' target='_blank' rel='noopener'>link</a>";
        if (f.archivo) html += "<span class='fuente-archivo'> · " + escHtml(f.archivo) + "</span>";
        html += "</li>";
      });
      html += "</ul></section>";
      bloques.push(html);
  ...
  function setVistaAgenda(vista) {
    ...
    var year = document.getElementById("cal-preview-year");
    ...
    var tYear = document.getElementById("tab-vista-anual");
    var esCal = vista === "calendario";
    var esLista = vista === "listado";
    var esAnual = vista === "anual";
    if (grid) grid.hidden = !esCal;
    if (lista) lista.hidden = !esLista;
    if (year) year.hidden = !esAnual;
    if (leyenda) leyenda.hidden = false;
    if (tCal) tCal.classList.toggle("active", esCal);
    if (tList) tList.classList.toggle("active", esLista);
    if (tYear) tYear.classList.toggle("active", esAnual);
  }
  ...
  function renderStatsChips(d) {
    ...
    var labels = {
      ...
      evento_local: "Bialet",
      vacaciones_invierno: "Invierno (prov.)",
      vacaciones_verano: "Verano escolar",
  ...
  function renderAgenda(d) {
    ...
    if (resumen) {
      resumen.textContent =
        ...
        " al " +
        hf +
        " — revisá la lista y borrá lo que no mueve cabañas";
    }
  ...
  var ESTADO_LABEL = {
    borrador: "Borrador",
    pendiente_aprobacion: "Revisar antes de publicar",
    aprobado: "Listo para publicar",
    publicado: "Publicado",
  };

  function cargarHoy() {
    var panel = document.getElementById("panel-hoy");
    var leyenda = document.getElementById("hoy-fecha-leyenda");
    if (!panel) return;
    fetch("/api/programa/hoy")
      .then(function (r) {
        if (!r.ok) throw new Error("sin api");
        return r.json();
      })
      .then(function (d) {
        if (leyenda) leyenda.textContent = d.fecha_hoy_legible || d.fecha_hoy;
        panel.innerHTML = "";
        if (d.sin_publicaciones_hoy) {
          panel.innerHTML =
            '<p class="hoy-vacio"><strong>Hoy no hay piezas en el calendario.</strong> Revisá Marketing o generá el rango que falte (Circuito 4).</p>';
        } else {
          (d.publicaciones_hoy || []).forEach(function (p) {
            var card = document.createElement("article");
            card.className = "hoy-card canal-" + (p.canal || "");
            var icon = CANAL_ICON[p.canal] || "📱";
            var estado = p.estado || "borrador";
            card.innerHTML =
              '<header class="hoy-card-head">' +
              "<span class='hoy-canal'>" +
              icon +
              " " +
              escHtml(p.canal_label || p.canal) +
              "</span>" +
              "<span class='hoy-hora'>" +
              escHtml(p.hora || "") +
              "</span>" +
              "<span class='hoy-estado estado-" +
              escHtml(estado) +
              "'>" +
              escHtml(ESTADO_LABEL[estado] || estado) +
              "</span>" +
              "</header>" +
              "<h3>" +
              escHtml(p.titulo || "Sin título") +
              "</h3>" +
              (p.tiene_video ? "<p class='hoy-video'>🎬 Video listo</p>" : "") +
              '<textarea class="hoy-copy" readonly rows="6">' +
              escHtml(p.copy || "") +
              "</textarea>" +
              '<div class="hoy-actions">' +
              '<button type="button" class="btn btn-primary btn-sm btn-copiar-hoy">Copiar texto</button>' +
              '<a class="btn btn-outline btn-sm" href="/marketing">Abrir Marketing</a>' +
              "</div>";
            var btn = card.querySelector(".btn-copiar-hoy");
            var ta = card.querySelector(".hoy-copy");
        ...
        var proxWrap = document.getElementById("panel-proximas");
        var proxList = document.getElementById("lista-proximas");
        if (proxWrap && proxList && d.proximas && d.proximas.length) {
          proxWrap.hidden = false;
          proxList.innerHTML = "";
          d.proximas.forEach(function (p) {
            var li = document.createElement("li");
            li.innerHTML =
              "<strong>" +
              fmtDDMMYYYY(p.fecha_publicacion) +
              " " +
              escHtml(p.hora || "") +
              "</strong> · " +
              escHtml(p.canal_label || p.canal) +
      ...
      .catch(function () {
        if (leyenda) leyenda.textContent = "Servidor apagado";
        panel.innerHTML =
          '<p class="hoy-vacio">Encendé el programa con el icono del escritorio para ver qué publicar hoy.</p>';
      });
  }
  ...
  function excluirEventoAgenda(it, btn) {
    if (!confirm("¿Sacar «" + (it.nombre || "") + "» del calendario? No se armarán posts por este evento.")) {
      return;
    }
    btn.disabled = true;
    fetch("/api/programa/eventos/excluir", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        nombre: it.nombre,
        fecha_inicio: (it.fecha_inicio || it.fecha || "").slice(0, 10),
        tipo: it.tipo,
        motivo: "Descartado desde lista del mes",
      }),
    })
      .then(function (r) {
        return r.json();
      ...
      .then(function (res) {
        ...
        mkToast(res.mensaje || "Evento excluido");
        return cargarPublicaciones();
      })
      .catch(function (e) {
        alert(e.message || "No se pudo excluir. ¿Servidor encendido?");
        btn.disabled = false;
      });
  }

  function abrirAgregarEventoManual() {
    var nombre = prompt("Nombre del evento (ej. Cosquín Rock, Rally, feriado puente…):");
    if (!nombre || !nombre.trim()) return;
    var fecha = prompt("Fecha inicio (AAAA-MM-DD):", inputDesde && inputDesde.value ? inputDesde.value.slice(0, 8) + "15" : "");
    if (!fecha) return;
    fetch("/api/programa/eventos/agregar", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        nombre: nombre.trim(),
      ...
      .then(function (res) {
        if (!res.ok) throw new Error(res.error || "error");
        mkToast(res.mensaje || "Evento agregado");
        return cargarPublicaciones();
      })
      .catch(function (e) {
  ...
  var btnAgregarEv = document.getElementById("btn-agregar-evento-manual");
  if (btnAgregarEv) {
    btnAgregarEv.addEventListener("click", abrirAgregarEventoManual);
  }
  ...
  function renderListado(d) {
    ...
    items.forEach(function (it) {
      ...
      var editable = it.editable !== false && it.tipo !== "feriado_nacional" && it.tipo !== "finde_largo";
      var acciones =
        '<div class="agenda-event-actions">' +
        (editable
          ? '<button type="button" class="btn-icon-excluir" title="Sacar del calendario">🗑</button>'
          : '<span class="agenda-event-locked" title="Feriado/puente oficial">🔒</span>') +
        "</div>";
      card.innerHTML =
        acciones +
        '<div class="card-fecha">' +
        escHtml(rangoTexto(it)) +
        "</div><div class='agenda-event-body'>" +
        ...
        (it.motivo_filtro && it.potencial_cabaña === false
          ? '<p class="pill-estado-warn">Filtrado: ' + escHtml(it.motivo_filtro) + "</p>"
          : "") +
        (resolveSourceUrl(it)
          ? '<p class="pill-estado-ok"><a href="' +
            escHtml(resolveSourceUrl(it)) +
            '" target="_blank" rel="noopener">Ver fuente del dato</a></p>'
          : "") +
        "</div>";
      var btnEx = card.querySelector(".btn-icon-excluir");
      if (btnEx) {
        btnEx.addEventListener("click", function () {
          excluirEventoAgenda(it, btnEx);
        });
      }
    ...
    if (!items.length) {
      lista.innerHTML =
        "<p class='agenda-lead'>No hay eventos activos en este mes. Usá ➕ Agregar evento o «Actualizar eventos» si falta un feriado.</p>";
    }
  }
  ...
  var btnMesAnt = document.getElementById("btn-mes-anterior");
  var btnMesSig = document.getElementById("btn-mes-siguiente");
  var btnMesHoy = document.getElementById("btn-mes-hoy");
  if (btnMesAnt) btnMesAnt.addEventListener("click", function () {
    cambiarMes(-1);
  });
  if (btnMesSig) btnMesSig.addEventListener("click", function () {
    cambiarMes(1);
  });
  if (btnMesHoy) btnMesHoy.addEventListener("click", function () {
    initMesActual();
    cargarPublicaciones().catch(function () {});
  });
  if (inputMes) {
  ...
  window.cargarHoy = cargarHoy;
  window.cargarEstadoPrograma = cargarEstado;
